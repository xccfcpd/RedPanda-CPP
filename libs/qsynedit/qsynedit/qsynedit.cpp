    ...
    for (int line=0;line<mDocument->count();line++) {
        if (mDocument->getSyntaxState(line)->hasTrailingSpaces) {
                ...
                addChangeToUndo(ChangeReason::Delete,
                        CharPos{(int)oldLine.length(), line},
                        CharPos{(int)newLine.length(), line},
                        QStringList({deleted}),
...
CharPos QSynEdit::prevWordEnd(const CharPos &pos) const
{
    ...
    if (pos.ch == 0) {
        ...
        p = prevSpaceChar(CharPos{(int)mDocument->getLine(pos.line).length()-1,pos.line});
        if (p.isValid())
...
void QSynEdit::selTokenAt(const CharPos &pos)
{
    ...
    if (getTokenAttriAtRowCol(pos, token, start, attr)) {
        if (!token.isEmpty()) {
            ...
            CharPos vWordEnd{start+(int)token.length(), pos.line};
...
void QSynEdit::doUndoRedoInput(const QString &oldGlyph,
                             ...
                             bool undo)
{
    ...
    QString replacedStr = lineStr.mid(
                changeStartPos.ch,
    ...
    QString restoredStr = leftStr+oldGlyph+rightStr;
    properSetLine(changeLine, restoredStr, true);
    if (undo) {
        mRedoList->addRedo(
                    ChangeReason::Input,
                    changeStartPos,
                    CharPos{changeStartPos.ch+(int)oldGlyph.length(),changeLine},
                    QStringList{replacedStr},
                    changeSelMode,
                    changeNumber);
    } else { //redo
...
void QSynEdit::doDeleteCurrentToken()
{
    ...
    if (getTokenAttriAtRowCol(pos, token, start, attr)) {
        if (!token.isEmpty()) {
            ...
            CharPos wordEnd{start+(int)token.length(), pos.line};
...
void QSynEdit::doDeleteToEOL()
{
    ...
    doDeleteText(caretXY(),CharPos{(int)lineText().length(),mCaretY},SelectionMode::Normal);
}
...
void QSynEdit::doSelectLine()
{
    ...
    if (mCaretY==mDocument->count()-1)
        ptEnd = CharPos{(int)lineText().length(),mCaretY};
    else
...
//    }
    ...
    setCaretXY(CharPos{(int)indentSpacesForRightLineText.length(),mCaretY + 1});
    updateLastCaretX();
}
...
void QSynEdit::internalInputStr(const QString& inputStr)
{
    if ((mActiveSelectionMode != SelectionMode::Normal)
            || selAvail()) {
        doSetSelText(inputStr);
        return;
    }
    Q_ASSERT(validGlyphStart(caretXY()));
    beginEditing();
    QString s = lineText();
    QString replaceStr;
    QString newS;
    if (!mInserting) {
        int inputGlyphCounts = calcGlyphStartCharList(inputStr).length();
        int currentGlyphIdx = mDocument->charToGlyphIndex(mCaretY, mCaretX);
        if (currentGlyphIdx < mDocument->glyphCount(mCaretY)) {
            int endDeleteGlyphIdx = std::min(currentGlyphIdx + inputGlyphCounts - 1, mDocument->glyphCount(mCaretY) - 1);
            int replaceEnd = mDocument->glyphEndChar(mCaretY, endDeleteGlyphIdx);
            replaceStr = s.mid(mCaretX, replaceEnd-mCaretX);
            newS = s.left(mCaretX)+inputStr+s.mid(replaceEnd);
        }
    }
    if (replaceStr.isEmpty()) {
        newS = s.left(mCaretX)+inputStr+s.mid(mCaretX);
    }
    properSetLine(mCaretY,newS,true);
    addChangeToUndo(ChangeReason::Input,
            CharPos{mCaretX, mCaretY},
            CharPos{mCaretX+(int)inputStr.length(), mCaretY},
            QStringList(replaceStr),
            SelectionMode::Normal
            );
    setCaretX(mCaretX+inputStr.length());
    endEditing();
}

void QSynEdit::doInputStr(const QString& s)
{
    if (mReadOnly)
        return;
    QString inputStr = s;
    ...
    if (inputStr.length() == 1 ) {
        if (!inputStr[0].isPrint() && inputStr[0]!='\t')
            return;
    }
                    ...
                    if (inputStr == ":") {
                        ...
                        if (mCaretX == line.length()) {
                            ...
                            if (indentSpaces != leftSpaces(line)) {
                                ...
                                setCaretXY(CharPos{(int)newLine.length(),oldCaretY});
                                addChangeToUndo(ChangeReason::Delete,
                                        CharPos{0, oldCaretY},
                                        CharPos{(int)line.length(), oldCaretY},
                                        QStringList(line),
                                        SelectionMode::Normal
                                        );
                                addChangeToUndo(ChangeReason::Insert,
                                        CharPos{0, oldCaretY},
                                        CharPos{(int)newLine.length(), oldCaretY},
                                        QStringList(),
                                        SelectionMode::Normal
                                        );
                            }
                        ...
                        if (left.trimmed().isEmpty()) {
                            int indentSpaces = calcIndentSpaces(oldCaretY,inputStr, true);
                            if (indentSpaces != leftSpaces(left)) {
                                ...
                                setCaretXY(CharPos{(int)newLeft.length(),oldCaretY});
                                addChangeToUndo(ChangeReason::Delete,
                                        CharPos{0, oldCaretY},
                                        CharPos{(int)left.length(), oldCaretY},
                                        QStringList(left),
                                        SelectionMode::Normal
                                        );
                                addChangeToUndo(ChangeReason::Insert,
                                        CharPos{0, oldCaretY},
                                        CharPos{(int)newLeft.length(), oldCaretY},
                                        QStringList(""),
                                        SelectionMode::Normal
                                        );
...
void QSynEdit::collapse(const PCodeBlock &foldRange)
{
    ...
    if ((mCaretY > foldRange->fromLine) && (mCaretY <= foldRange->toLine)) {
          setCaretXY(CharPos{(int)mDocument->getLine(foldRange->fromLine).length(),
                                 foldRange->fromLine});
    }
...
inline bool isGroupChange(ChangeReason reason){
    return (reason == ChangeReason::Input
    ...
    if (item) {
        ...
        case ChangeReason::InsertLine:
            ...
            mRedoList->addRedo(
                        ...
                        item->changeNumber());
            break;
        case ChangeReason::Input: {
            doUndoRedoInput(item->changeText()[0],
                          item->changeStartPos(),
                          item->changeEndPos(),
                          item->changeSelMode(),
                          item->changeNumber(),
    ...
    while(line<=endLine) {
        ...
        if (!mUndoing) {
            addChangeToUndo(ChangeReason::Insert,
                    ...
                    CharPos{insertPos+(int)str.length(), line},
                    QStringList(),
                    SelectionMode::Normal);
        }
        if (i<text.length()-1) {
    ...
    if (event->key() == Qt::Key_Escape && mActiveSelectionMode != SelectionMode::Normal) {
        ...
        if (cmd!=EditCommand::None) {
            ...
            if (event->text().length()==1) {
                ...
                if (c=='\t' || c.isPrint()) {
                    processCommand(EditCommand::Input,event->text());
                    event->accept();
                }
            } else  {
                processCommand(EditCommand::Input,event->text());
...
//    }
    ...
    if (lastLineUsed) {
        ...
        QString s=mDocument->getLine(line);
        addChangeToUndo(ChangeReason::LineBreak,
                         CharPos{(int)s.length(),line},
                         CharPos{(int)s.length(),line}, QStringList(), SelectionMode::Normal);
    } else
        properDeleteLine(mDocument->count()-1, true);
    addLeftTopToUndo();
    internalSetCaretXY(coord);
    if (event->proposedAction() == Qt::DropAction::CopyAction) {
        ...
        if (coord < mDragSelBeginSave ) {
            ...
            if (coord.line>mDocument->count()) {
                ...
                addChangeToUndo(ChangeReason::LineBreak,
                                     CharPos{(int)s.length()+1,line},
                                     CharPos{(int)s.length()+1,line}, QStringList(), SelectionMode::Normal);
                endEditing();
                coord.line = line;
                coord.ch=0;
            } else {
